...
public:
	...
	static void logOutput(){
		...
		output << simgrid::s4u::Engine::getClock() << ";";
...
static void shutDownRennesHosts(std::vector<std::string> args){
	...
	simgrid::s4u::Actor::byPid(pid_on)->kill();
...
static int master_nominal_behavior(std::vector<std::string> args){
	...
	while(running_vms.size()<max_vms){
		...
		for(int i=0;i<nb_simultaneous_deployments;i++){
			...
			vm->setRamsize(vm_ram_size);
			vm->start();

			for(int j=0;j<vm_core_nb;j++){
				std::string actor_name = std::to_string(j) + "_" + vm_name;
				simgrid::s4u::Actor::createActor(vm_name.c_str(), vm , slave_behavior, args);
			}
...
static int failureNotifier(std::vector<std::string> args){
	...
	*payload        = simgrid::s4u::Engine::getClock();
	...
	simgrid::s4u::Mailbox::byName("emergency")->put(payload,200);
...
static int failureManager(std::vector<std::string> args){
	...
	simgrid::s4u::Mailbox::byName("emergency")->get();

	unsigned long pid_current_manager = std::stoul(args[0],0,10);

	// stop deploying VMs you @%^&"A§* !!!!
	simgrid::s4u::Actor::byPid(pid_current_manager)->kill();
	...
	for(simgrid::s4u::VirtualMachine* vm : running_vms){
		...
		XBT_INFO("start migration of vm %s (status %i) to host %s",vm->getName().c_str(),vm->getState(),dest->getName().c_str());
		sg_vm_migrate(vm, dest);
		XBT_INFO("end of migration of vm %s to host %s",vm->getName().c_str(),dest_name.c_str());
...
int main(int argc, char *argv[])
{
  ...
  e.loadPlatform("clusters_rennes.xml");
  ...
  std::string fmu_uri_2 = "file:///home/mecsyco/Documents/thermal_system";
  ...
  simgrid::s4u::ActorPtr master_nominal = simgrid::s4u::Actor::createActor("master_nominal", simgrid::s4u::Host::by_name("c-0.sophia"), master_nominal_behavior, args);
  simgrid::s4u::Actor::createActor("failure_notifier", simgrid::s4u::Host::by_name("c-0.rennes"), failureNotifier, args);
  std::vector<std::string> args_failure_manager = {std::to_string(master_nominal.get()->getPid())};
  simgrid::s4u::ActorPtr failure_manager = simgrid::s4u::Actor::createActor("failure_manager", simgrid::s4u::Host::by_name("c-0.sophia"), failureManager, args_failure_manager);

  std::vector<std::string> args_shutdown = {std::to_string(failure_manager.get()->getPid())};
  simgrid::s4u::Actor::createActor("shutdown_process", simgrid::s4u::Host::by_name("c-0.sophia"), shutDownRennesHosts, args_shutdown);
